...
import { SubmitQuizReplyRequest } from "@stores/game.dto";
...
function QuizReplySection({
  ...
  const { placeholder, sendRandomWordReplyToServer } =
    useZeroRound(emitSubmitQuizReply);
  const [isRoundTimeout, setIsRoundTimeout] =
    useRecoilState(isRoundTimeoutState);
      ...
      {!isDraw ? (
        <>
          ...
          <AnswerInput
            ...
            onKeyUp={(e: any) => checkEnterKeyAndSubmit(e)}
...
const AnswerInput = styled.input<{ quizSubmitted: boolean }>`
  ...
  color: ${(props) =>
    props.quizSubmitted ? props.theme.color.gray1 : props.theme.color.green};
