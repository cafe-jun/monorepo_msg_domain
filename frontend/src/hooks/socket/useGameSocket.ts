...
import { EmitLeaveGameRequest, JoinLobbyReEmitRequest } from "@stores/user.dto";
...
function useGameSocket(setIsCompleteGame: SetterOrUpdater<boolean>) {
  ...
  const setSubmittedQuizReplyCount = useSetRecoilState(
    submittedQuizReplyCountState
  );
  ...
  function onCompleteGame() {
    NetworkService.on(
      "complete-game",
      (gameResult: CompleteGameEmitRequest) => {
